...
//	printf("serverSocket is %d\n", serverSocket);
  ...
  /*QUESTION 6*/
	...
	int err_check = getaddrinfo(host, port, &hints, &result);
	if(err_check != 0){
  ...
  /*QUESTION 7*/
	...
	int connect_check = connect(serverSocket, result->ai_addr, result->ai_addrlen);
	if(connect_check != 0){
		perror(NULL);
		exit(1);
	}

//	printf("Client should've connected\n");
